          ...
          <div class="content-container view-list">
            ...
            <div class="promo-content-list-bar">
              <div class="search-filter-bar contents-cards-bar">
                <form action="" method="get">
                  <input type="hidden" name="action" value="stand-by-list" />
                ...
                <?php if (isset($apprenants) && is_array($apprenants)): ?>
                  <?php foreach ($apprenants as $apprenant): ?>
                    ...
                    <tr>
                      ...
                      <td><?= isset($apprenant['prenom']) ? htmlspecialchars($apprenant['prenom']) : '' ?></td>
                      <td><?= isset($apprenant['nom']) ? htmlspecialchars($apprenant['nom']) : '' ?></td>
                      <td><?= isset($apprenant['date_naiss']) ? htmlspecialchars($apprenant['date_naiss']) : '' ?></td>
                      <td><?= isset($apprenant['lieu_naiss']) ? htmlspecialchars($apprenant['lieu_naiss']) : '' ?></td>
                      <td><?= isset($apprenant['adresse']) ? htmlspecialchars($apprenant['adresse']) : '' ?></td>
                      <td><?= isset($apprenant['email']) ? htmlspecialchars($apprenant['email']) : '' ?></td>
                      ...
                      <td><span class="action"><a href="<?= Routes::APPRENANT->resolve() ?>?action=edit&appr=<?= $apprenant['id'] ;?>">Corriger</a></span></td>
                    </tr>
                  <?php endforeach; ?>
                <?php else: ?>
                  <tr>
            ...
            <div class="pagination">
              ...
              <div class="page-nav">
                ...
                ?>
                <?php if ($page > 1): ?>
                  <a href="?action=stand-by-list&page=<?= $page - 1 ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>&view=<?= $view ?>" class="page-btn">Précédent</a>
                <?php endif; ?>

                <?php for ($i = 1; $i <= $pages; $i++): ?>
                  <a href="?action=stand-by-list&page=<?= $i ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>&view=<?= $view ?>" class="page-btn <?= $i === $page ? 'active' : '' ?>">
                    <?= $i ?>
                  </a>
                <?php endfor; ?>

                <?php if ($page < $pages): ?>
                  <a href="?action=stand-by-list&page=<?= $page + 1 ?>&filter=<?= urlencode($filter) ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>&view=<?= $view ?>" class="page-btn">Suivant</a>
                <?php endif; ?>
              </div>
            </div>
            </div>
